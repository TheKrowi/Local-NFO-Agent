...
class LocalNFO(PlexAgent):
    """
    A Plex Metadata Agent for Movies.

    Uses nfo files as the metadata source for Plex Movies.
    """
    name = 'LocalNFO'
    ver = '0.1'
        ...
        if nfo_file:
            ...
            if nfo_text_lower.count('<movie') > 0 and nfo_text_lower.count('</movie>') > 0:
                ...
                if not preferences['localmediaagent'] and preferences['subtitle']:
                    # Subtitle Support
                    # Supports Plex subtitle filenames
                    subtitle_files = []
...
class LocalNFOLogAdapter(PlexLogAdapter):
    """
